...
Another usage of the FROM clause specifies a path within a bucket.  For example:
    ...
    FROM organizations.employees

In this example, a bucket named `organizations` contains documents describing each organization.  Each organization document has a field namd `employees` which is an array of employee object.  Using this syntax each employee object becomes the input to the query.
...
Finally, the most complex usage of the FROM clause allows for path joins within a document.  This feature is sometimes referred to as `OVER`, `FLATTEN` or `UNNEST`.  Conceptually, a document containing a nested array can have each of the members of this array joined with the document.  Each of these joined objectes become the input to the query.
    ...
    FROM organizations AS organization PATH JOIN organization.employees AS employee
...
As the right hand side of a PATH JOIN is another data source, these PATH JOINS may be chained together.  However, the PATH JOIN argument must always be path referring to the immediate left hand `AS` clause.  The following is a valid usage of multiple PATH JOINs.

    FROM organization AS organizations PATH JOIN organization.employees AS employee PATH JOIN employee.addresses AS address
...
## Appendix 1 - Identifier Ambiguity
