...
var validQueries = []string{
	...
	`SELECT * FROM abucket WHERE ANY child.age > 25 OVER child IN children END`,
	`SELECT * FROM abucket WHERE ALL child.age > 25 OVER child IN children END`,
	`SELECT * FROM abucket WHERE ALL child.age > 25 OVER child IN children[0] END`,
	`SELECT * FROM abucket WHERE ALL child.age > 25 OVER child IN a.children[0] END`,
	`SELECT * FROM abucket WHERE ALL child.age > 25 OVER child IN b.a.children[0] END`,
	`SELECT * FROM abucket WHERE ALL child.age > 25 OVER child IN b.a.children END`,
	`SELECT * FROM abucket WHERE ALL child.age > 25 OVER child IN some.my.children END`,
	"SELECT * FROM abucket WHERE ALL child.age > 25 OVER child IN `all`.my.children END",
	`SELECT * FROM abucket WHERE ANY child.age > 25 OVER child IN children END AND age > 65 OR color = "blue"`,
	...
	`SELECT * FROM contacts WHERE ANY child.age > 14 AND child.gender IS NOT NULL OVER child IN children END`,
	`SELECT * FROM abucket WHERE FIRST child OVER child IN children WHEN child.age > 25 END`,
	`SELECT ARRAY child.name OVER child IN contacts.children WHEN child.age > 20 END FROM contacts`,
	...
	`SELECT * FROM tutorial WHERE ANY predicate OVER name IN collection END`,
	`SELECT * FROM tutorial WHERE FIRST expression OVER name IN collection END`,
	`SELECT * FROM tutorial WHERE FIRST expression OVER name IN collection WHEN predicate END`,
	`SELECT * FROM tutorial WHERE ARRAY expression OVER name IN collection END`,
	`SELECT * FROM tutorial WHERE ARRAY expression OVER name IN collection WHEN predicate END`,
	...
	`CREATE INDEX abv_idx ON :pool.beer-sample(abv)`,
	`CREATE INDEX abv_idx ON :pool.beer-sample(abv) USING VIEW`,
	`CREATE INDEX abv_idx ON :pool.beer-sample(abv) USING magic`,
	...
	`CREATE INDEX abv_idx ON :pool.beer-sample(abv, ibu)`,
	`CREATE INDEX abv_idx ON :pool.beer-sample(abv, ibu) USING VIEW`,
	`CREATE INDEX abv_idx ON :pool.beer-sample(abv, ibu) USING magic`,
	...
	`CREATE PRIMARY INDEX ON :pool.beer-sample`,
	`CREATE PRIMARY INDEX ON :pool.beer-sample USING VIEW`,
	`CREATE PRIMARY INDEX ON :pool.beer-sample USING magic`,
...
func TestParserASTOutput(t *testing.T) {
	...
	tests := []struct {
		...
		{"DROP INDEX :pool.beer-sample.abv",
			&ast.DropIndexStatement{
				Pool:   "pool",
				Bucket: "beer-sample",
				Name:   "abv",
